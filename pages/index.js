...
export default function Home() {
  ...
  const submitHandle = (e) => {
    ...
    fetch('/api/generate-regex', {
      ...
      .then((res) => {
        if (res?.error) {
          setError(
            'Regex deseni üretemiyorum, doğru bir şey istediğinden emin misin? LAN.'
          )
          setResult(false)
        } else {
          setResult(res)
  ...
  return (
    <div className="container mx-auto my-6">
      ...
      {result && (
        <div>
          <h1 className="text-2xl font-medium text-zinc-400 mb-4">
            {result.description}
          </h1>
          <div className="bg-black text-white rounded-md mb-6">
            <h6 className="pt-4 pl-4 font-medium text-xs text-zinc-500">
              Regex Deseni
            </h6>
          ...
          <pre className="bg-black p-4 rounded-md font-mono text-white text-[15px]">
            {result.example}
          </pre>
        </div>
      )}
